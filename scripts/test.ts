...
console.log('\n── data / materialize ──');
ok('905 base rows', base.length === 905);
ok('all ids unique', new Set(base.map((t) => t.id)).size === 905);
{
...
console.log('\n── analytics ──');
eq('net total (incl transfer)', grandTotal(toSpendingEvents(txns)), 226826.05, 0.5);
{
  ...
  eq('essential (+ Grab rides)', g.essential, 79170.40);
  eq('discretionary net (- Grab rides)', g.discretionary, 96151.84);
  eq('transfer', g.transfer, 51503.81);
  eq('net unchanged by reclassification', g.essential + g.discretionary + g.transfer, 226826.05, 1);
}
{
...
}
{
  ...
  eq('dailySpending sums to net total', sum, 226826.05, 1);
...
console.log('\n── import / export (io) ──');
{
  ...
  ok('re-import all -> all duplicates', res.duplicates === 905);
...
ok('settlement is transfer group', categoryGroup('ชำระบัตรเครดิต') === 'transfer');
{
  ...
  eq('card-bill payment excluded from net (no double count)', grandTotal(toSpendingEvents(m)), 226826.05, 1);
