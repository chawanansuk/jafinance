...
console.log('\n── data / materialize ──');
ok('1304 base rows', base.length === 1304);
ok('all ids unique', new Set(base.map((t) => t.id)).size === 1304);
{
...
console.log('\n── analytics ──');
eq('net total (incl transfer)', grandTotal(toSpendingEvents(txns)), 316319.82, 0.5);
{
  ...
  eq('discretionary net (- Grab rides)', g.discretionary, 146332.03);
  eq('transfer (excl card settlement)', g.transfer, 65668.68);
  eq('net unchanged by reclassification', g.essential + g.discretionary + g.transfer, 316319.82, 1);
}
{
...
}
{
  ...
  eq('dailySpending sums to net total', sum, 316319.82, 1);
...
console.log('\n── import / export (io) ──');
{
  ...
  ok('re-import all -> all duplicates', res.duplicates === 1304);
...
ok('settlement is transfer group', categoryGroup('ชำระบัตรเครดิต') === 'transfer');
{
  ...
  eq('card-bill payment excluded from net (no double count)', grandTotal(toSpendingEvents(m)), 316319.82, 1);
